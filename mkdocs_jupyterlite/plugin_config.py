...
class LiteConfig(Config):
    ...
    mounts = config_options.ListOfItems(
        config_options.SubConfig(MountConfig), default=[])
    ...
    kernel_mapping = config_options.DictOfItems(
        config_options.Type(str), default={'py': 'xpython'})
    ...
    def hash(self, name):
        ...
        mounts_str = ','.join(
            [f"{mount.host}:{mount.target}" for mount in self.mounts])
...
class JupyterlitePluginConfig(Config):
    environments = config_options.DictOfItems(
        config_options.SubConfig(LiteConfig))  # required
