...
class JupyterlitePlugin(BasePlugin[JupyterlitePluginConfig]):
        ...
        for lite_env_name, lite_env_config in lite_envs.items():
            ...
            for item in self._env_notebook_files[lite_env_name]:
                ...
                with open(markdown_path) as f:
                    ...
                    content = content + \
                        f"[run notebook]({go_up}{lite_env_name}/lite/lab/index.html?path={item_name}.ipynb&kernel={kernel_name})"
                ...
                if False:
                    ...
                    file = mkdocs.structure.files.File(
                        path=self._docs_path(
                            lite_env_name) / f"{item_name}.{extenstion}.md",
                        src_dir=self._env_notebook_markdown_dir_root(
                            lite_env_name),
    ...
    def on_config(self, config):
        ...
        for lite_env_name, lite_env_config in lite_envs.items():
            ...
            notebook_dir = self.config['environments'][lite_env_name].get(
                'notebook_dir')
            notebook_pattern = self.config['environments'][lite_env_name].get(
                'notebook_pattern')
            kernel_mapping = self.config['environments'][lite_env_name].get(
                'kernel_mapping')

            notebooks = convert_notebooks(
                notebook_dir=notebook_dir, notebook_pattern=notebook_pattern,
    ...
    def on_post_build(self, config):
        ...
        for lite_env_name, lite_env_config in lite_envs.items():
            ...
            build_jupyterlite(
                config=config, lite_env_name=lite_env_name,
            ...
            page_lite_dir = Path(config.get('site_dir')) / \
                lite_env_name / 'lite'
